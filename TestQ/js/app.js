...
import { $, getVal, setVal, setupNumericFormatting } from './utils.js';
...
export class App {
  ...
  init() {
    ...
    initialData.paymentTiming ||= 'advance';
...
window.onGuarCheck = (cb) => {
  ...
  recalculate() {
    try {
      ...
      // 5. Auto-save
      this.storage.save({ ...inputs, flatRate: getVal('flatRate') });
    } catch (error) {
...
async exportExcel() {
  ...
  resetAll() {
    ...
    setVal('paymentTiming', DEFAULT_VALUES.paymentTiming || 'advance');
    this.recalculate();
